...
#undef ADD_WT
        ...
        __device__ __inline__ void _dda_unit_LOT(
...
#pragma unroll
        ...
        __device__ __inline__ void clamp_coord_LOT(float *__restrict__ q)
        {
            for (int i = 0; i < 3; ++i)
            {
                q[i] = max(float(0.0), min(float(1.0) - 1e-6, q[i]));
            }
        }

        __device__ __inline__ void query_single_from_root_LOT(
            const int32_t *__restrict__ child,
            const int32_t *__restrict__ node_corner,
            float *__restrict__ xyz_inout,
            float *__restrict__ cube_sz_out,
            int32_t *__restrict__ node_corner_ids)
        {

            const float N = 2;

            clamp_coord_LOT(xyz_inout);
            ...
            *cube_sz_out = N;
            while (true)
            {
                ...
                int64_t id_ptr_index = node_id * int64_t(N * N * N) +
                               u * int32_t(N * N) + v * int32_t(N) + w;

                const int32_t skip = child[id_ptr_index];
                if (skip == 0)
                {
                    for (int32_t i = 0; i < 8; ++i)
                    {
                        int64_t index = int64_t(id_ptr_index * 8) + i;
                        node_corner_ids[i] = node_corner[index];
                    }
        ...
        __device__ __inline__ void query_single_from_root_downsample_LOT(
            const int32_t *__restrict__ child,
            const int32_t *__restrict__ node_corner,
            float *__restrict__ xyz_inout,
            float *__restrict__ cube_sz_out,
            int64_t *__restrict__ out_node_id)
        {
        ...
        __device__ __inline__ void query_single_from_root_refine_LOT(
            const int32_t *__restrict__ child,
            const int32_t *__restrict__ node_corner,
            float *__restrict__ xyz_inout,
            float *__restrict__ cube_sz_out,
            ...
            int32_t *__restrict__ node_id_out)
        {
            ...
            *cube_sz_out = N;
            while (true)
            {
                ...
                int64_t id_ptr_index = node_id * N * N * N + u * N * N + v * N + w;
                ...
                if (skip == 0)
                {
                    for (int i = 0; i < 8; ++i)
                    {
                        node_corner_ids[i] = node_corner[id_ptr_index * 8 + i];
                    }

                    node_id_out[0] = node_id;
                    node_id_out[1] = u;
                    node_id_out[2] = v;
                    node_id_out[3] = w;
        ...
        __device__ __inline__ float diff_psdf(
            float psdf,
            float nsdf,
            float learn_S)
        {
            double esn = _EXP(learn_S * nsdf);
            double esp = _EXP(learn_S * psdf);

            double diff_p = (learn_S * esn) /
                            ((esn + 1) * esp);
            return diff_p;
        }

        __device__ __inline__ float diff_nsdf(
            float psdf,
            float nsdf,
            float learn_S)
        {
            ...
            double diff_n = -(learn_S * esn * (1 + esp)) /
                            ((esn + 1) * (esn + 1) * esp);
            return diff_n;
        }

        __device__ __inline__ float diff_ssdf(
            float psdf,
            float nsdf,
            float learn_S)
        {
            ...
            double diff_s = (esn * (psdf - nsdf + psdf * esn - nsdf * esp)) / (esp * (esn + 1) * (esn + 1));
            return diff_s;
        }
...
#undef MAYBE_READ_LINK
        ...
        __device__ __inline__ float trilerp_cuvol_one_LOT(
            const int32_t *__restrict__ node_corner_ids,
            const data_type_t *__restrict__ data,
            int32_t stride, const float *__restrict__ pos,
            const int32_t idx)
        {
            data_type_t c_data[8];
            for (int i = 0; i < 8; ++i)
            {
                int64_t index = node_corner_ids[i] * stride + idx;
                c_data[i] = data[index];
            }

            const float c00 = lerp(c_data[0], c_data[2], pos[0]);
            const float c01 = lerp(c_data[4], c_data[6], pos[0]);
            const float c10 = lerp(c_data[1], c_data[3], pos[0]);
            const float c11 = lerp(c_data[5], c_data[7], pos[0]);
            const float c0 = lerp(c00, c10, pos[1]);
            const float c1 = lerp(c01, c11, pos[1]);
            const float f_c = lerp(c0, c1, pos[2]);

            return f_c;
        }

        template <class data_type_t>
        __device__ __inline__ float trilerp_eikonal_one_LOT(
            const int32_t *__restrict__ node_corner_ids,
            const data_type_t *__restrict__ data,
            const int32_t dir_index,
            size_t stride, const float *__restrict__ pos,
            const int idx)
        {
            data_type_t c_data[8];
            for (int i = 0; i < 8; ++i)
            {
                c_data[i] = data[node_corner_ids[i] * stride + idx];
            }

            float pos_x1 = 1.0 - pos[0];
            float pos_y1 = 1.0 - pos[1];
            float pos_z1 = 1.0 - pos[2];

            float f_c;
            if (dir_index == 0)
            {
                float c0 = -pos_y1 * pos_z1 * c_data[0];
                float c1 = -pos[1] * pos_z1 * c_data[1];
                float c2 = pos_y1 * pos_z1 * c_data[2];
                float c3 = pos[1] * pos_z1 * c_data[3];
                float c4 = -pos_y1 * pos[2] * c_data[4];
                float c5 = -pos[1] * pos[2] * c_data[5];
                float c6 = pos_y1 * pos[2] * c_data[6];
                float c7 = pos[1] * pos[2] * c_data[7];

                f_c = c0 + c1 + c2 + c3 + c4 + c5 + c6 + c7;
            }

            if (dir_index == 1)
            {
                float c0 = -pos_x1 * pos_z1 * c_data[0];
                float c1 = pos_x1 * pos_z1 * c_data[1];
                float c2 = -pos[0] * pos_z1 * c_data[2];
                float c3 = pos[0] * pos_z1 * c_data[3];
                float c4 = -pos_x1 * pos[2] * c_data[4];
                float c5 = pos_x1 * pos[2] * c_data[5];
                float c6 = -pos[0] * pos[2] * c_data[6];
                float c7 = pos[0] * pos[2] * c_data[7];
            ...
            if (dir_index == 2)
            {
                float c0 = -pos_x1 * pos_y1 * c_data[0];
                float c1 = -pos_x1 * pos[1] * c_data[1];
                float c2 = -pos[0] * pos_y1 * c_data[2];
                float c3 = -pos[0] * pos[1] * c_data[3];
                float c4 = pos_x1 * pos_y1 * c_data[4];
                float c5 = pos_x1 * pos[1] * c_data[5];
                float c6 = pos[0] * pos_y1 * c_data[6];
                float c7 = pos[0] * pos[1] * c_data[7];
...
#undef MAYBE_ADD_LINK
        ...
        template <class data_type_t>
        __device__ __inline__ void trilerp_backward_cuvol_one_LOT(
            const int32_t *__restrict__ node_corner_ids,
            data_type_t *__restrict__ grad_data,
            size_t stride, const float *__restrict__ pos,
            float grad_out,
            const int idx)
        {
            ...
            atomicAdd(&grad_data[node_corner_ids[0] * stride + idx], xo * ay * az * grad_out);
            atomicAdd(&grad_data[node_corner_ids[1] * stride + idx], xo * pos[1] * az * grad_out);
            atomicAdd(&grad_data[node_corner_ids[2] * stride + idx], pos[0] * ay * az * grad_out);
            atomicAdd(&grad_data[node_corner_ids[3] * stride + idx], pos[0] * pos[1] * az * grad_out);
            atomicAdd(&grad_data[node_corner_ids[4] * stride + idx], xo * ay * pos[2] * grad_out);
            atomicAdd(&grad_data[node_corner_ids[5] * stride + idx], xo * pos[1] * pos[2] * grad_out);
            atomicAdd(&grad_data[node_corner_ids[6] * stride + idx], pos[0] * ay * pos[2] * grad_out);
            atomicAdd(&grad_data[node_corner_ids[7] * stride + idx], pos[0] * pos[1] * pos[2] * grad_out);
        }
...
#undef MAYBE_ADD_LINK_DEN
        ...
        template <class data_type_t>
        __device__ __inline__ void trilerp_backward_cuvol_one_density_LOT(
            const int32_t *__restrict__ node_corner_ids,
            data_type_t *__restrict__ grad_data_out,
            const float *__restrict__ pos,
            float grad_out)
        {
            ...
            atomicAdd(&grad_data_out[node_corner_ids[0]], xo * ay * az * grad_out);
            atomicAdd(&grad_data_out[node_corner_ids[1]], xo * pos[1] * az * grad_out);
            atomicAdd(&grad_data_out[node_corner_ids[2]], pos[0] * ay * az * grad_out);
            atomicAdd(&grad_data_out[node_corner_ids[3]], pos[0] * pos[1] * az * grad_out);
            atomicAdd(&grad_data_out[node_corner_ids[4]], xo * ay * pos[2] * grad_out);
            atomicAdd(&grad_data_out[node_corner_ids[5]], xo * pos[1] * pos[2] * grad_out);
            atomicAdd(&grad_data_out[node_corner_ids[6]], pos[0] * ay * pos[2] * grad_out);
            atomicAdd(&grad_data_out[node_corner_ids[7]], pos[0] * pos[1] * pos[2] * grad_out);
        }

        template <class data_type_t>
        __device__ __inline__ void trilerp_backward_eikonal_LOT(
            const int32_t *__restrict__ node_corner_ids,
            const int32_t dir_index,
            data_type_t *__restrict__ grad_data_out,
            const float *__restrict__ pos,
            float grad_out)
        {
            ...
            if (dir_index == 0)
            {
                atomicAdd(&grad_data_out[node_corner_ids[0]], -pos_y1 * pos_z1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[1]], -pos[1] * pos_z1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[2]], pos_y1 * pos_z1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[3]], pos[1] * pos_z1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[4]], -pos_y1 * pos[2] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[5]], -pos[1] * pos[2] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[6]], pos_y1 * pos[2] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[7]], pos[1] * pos[2] * grad_out);
            }

            if (dir_index == 1)
            {
                atomicAdd(&grad_data_out[node_corner_ids[0]], -pos_x1 * pos_z1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[1]], pos_x1 * pos_z1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[2]], -pos[0] * pos_z1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[3]], pos[0] * pos_z1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[4]], -pos_x1 * pos[2] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[5]], pos_x1 * pos[2] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[6]], -pos[0] * pos[2] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[7]], pos[0] * pos[2] * grad_out);
            }

            if (dir_index == 2)
            {
                atomicAdd(&grad_data_out[node_corner_ids[0]], -pos_x1 * pos_y1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[1]], -pos_x1 * pos[1] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[2]], -pos[0] * pos_y1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[3]], -pos[0] * pos[1] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[4]], pos_x1 * pos_y1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[5]], pos_x1 * pos[1] * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[6]], pos[0] * pos_y1 * grad_out);
                atomicAdd(&grad_data_out[node_corner_ids[7]], pos[0] * pos[1] * grad_out);
            }
        }

        template <class data_type_t>
        __device__ __inline__ void trilerp_backward_cuvol_one_eikonal_LOT(
            data_type_t *__restrict__ grad_data_out,
            const float *__restrict__ pos,
            float grad_out)
        {
            ...
            grad_data_out[0] = xo * ay * az * grad_out;
            grad_data_out[1] = xo * pos[1] * az * grad_out;
            grad_data_out[2] = pos[0] * ay * az * grad_out;
            grad_data_out[3] = pos[0] * pos[1] * az * grad_out;
            grad_data_out[4] = xo * ay * pos[2] * grad_out;
            grad_data_out[5] = xo * pos[1] * pos[2] * grad_out;
            grad_data_out[6] = pos[0] * ay * pos[2] * grad_out;
            grad_data_out[7] = pos[0] * pos[1] * pos[2] * grad_out;
        }
...
#define MAYBE_ADD_LINK2(u, valexpr)                                                      \
    {                                                                                    \
        ...
        {                                                                                \
            link *= nlayers;                                                             \
            float *__restrict__ gdptr = &grad_data_out[(link + l[2]) * nchannels + idx]; \
...
#pragma unroll
        ...
        __device__ __inline__ void calc_sphfunc_LOT(
            const PackedTreeSpecLOT &lotree,
            const float *__restrict__ dir, // Pre-normalized
            float *__restrict__ out)
        {
            // Placeholder
            calc_sh(lotree.basis_dim, dir, out);
        }
...
#pragma unroll 3
        ...
        __device__ __inline__ void ray_find_bounds_LOT(SingleRaySpec &__restrict__ ray, const PackedTreeSpecLOT &__restrict__ lotree)
        {
            transform_coord(ray.origin, lotree._scaling, lotree._offset);

            float delta_scale = _get_delta_scale(lotree._scaling, ray.dir);
            ray.delta_scale = delta_scale;

            float tmin, tmax;
            for (int i = 0; i < 3; ++i)
            {
                ray.invdir[i] = 1.0 / (ray.dir[i] + 1e-9);
            }
            _dda_unit_LOT(ray.origin, ray.invdir, &tmin, &tmax);
            ray.tmin = tmin;
            ray.tmax = tmax;
        }
